...
const stats = [
  { label: 'Total Dealers', value: '142', change: '+8', up: true, icon: Users, color: '#f3eeff', iconColor: '#a855f7' },
  { label: 'Active Orders', value: '38', change: '+5', up: true, icon: Package, color: '#fff0f9', iconColor: '#ec4899' },
  { label: 'Monthly Revenue', value: '₹4.2L', change: '+12%', up: true, icon: TrendingUp, color: '#f0fdf4', iconColor: '#16a34a' },
  { label: 'Pending Bills', value: '9', change: '-3', up: false, icon: FileText, color: '#fffbeb', iconColor: '#d97706' },
  { label: 'Zones Covered', value: '7', change: '+1', up: true, icon: Map, color: '#eff6ff', iconColor: '#2563eb' },
  { label: 'Super Stockists', value: '12', change: '0', up: true, icon: Star, color: '#fdf4ff', iconColor: '#c026d3' },
];

const recentActivity = [
  { icon: CheckCircle, color: '#16a34a', text: 'Dealer "Sharma Traders" onboarded in Zone A', time: '10 min ago' },
  { icon: AlertCircle, color: '#d97706', text: 'Stock alert: Face Serum inventory below 50 units', time: '35 min ago' },
  { icon: Clock, color: '#2563eb', text: 'Invoice #1042 generated for Ravi Distribution', time: '1 hr ago' },
  { icon: CheckCircle, color: '#16a34a', text: 'Area Zone D allocated to Super Stockist Mehta Co.', time: '2 hr ago' },
  { icon: AlertCircle, color: '#e11d48', text: 'Sub-dealer license renewal due: Krishna Stores', time: '3 hr ago' },
];
        ...
        <div className="dd-card-body" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {recentActivity.map((a, i) => (
            <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
              <a.icon size={16} color={a.color} style={{ marginTop: 2, flexShrink: 0 }} />
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: '0.82rem', color: '#374151' }}>{a.text}</p>
                <p style={{ fontSize: '0.72rem', color: '#9ca3af', marginTop: 2 }}>{a.time}</p>
              </div>
            </div>
          ))}
        </div>
